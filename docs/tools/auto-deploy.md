# 🚀 Script Ultra-Automatique avec API GitHub

Ce script utilise l'API GitHub pour télécharger les Dockerfiles (évite les erreurs 404 du CDN) et crée les fichiers manquants en fallback.
...
# Utiliser l'API GitHub pour télécharger les Dockerfiles existants
echo "  • Téléchargement des Dockerfiles via API GitHub..."
GITHUB_API="https://api.github.com/repos/romain-38530/RT-Technologie"
BRANCH="dockerfile"

# Fonction pour télécharger un fichier via API GitHub
download_github_file() {
  local file_path=$1
  local output_path=$2

  # Récupérer le contenu via l'API GitHub (évite le CDN)
  curl -s -H "Accept: application/vnd.github.v3.raw" \
    "$GITHUB_API/contents/$file_path?ref=$BRANCH" \
...
# Télécharger les Dockerfiles existants via API si disponibles
echo "  • Vérification des Dockerfiles existants via API..."
for s in "${!SERVICES[@]}"; do
  if download_github_file "services/$s/Dockerfile" "services/$s/Dockerfile.api"; then
    mv "services/$s/Dockerfile.api" "services/$s/Dockerfile"
    echo "    ✓ $s (téléchargé via API)"
  fi
done
...
echo ""
echo "🔧 ÉTAPE 2/6: Création des Dockerfiles manquants..."
echo "════════════════════════════════════════════════════════════════"
...
for s in "${!SERVICES[@]}"; do
  ...
  # Si le Dockerfile n'existe pas déjà, le créer
...
DOCKERFILE
    ...
    echo "  ✓ $s (créé localement)"
  else
    echo "  ✓ $s (déjà existant via API)"
  fi
done

echo "  ✓ Tous les Dockerfiles prêts"
...
Le script va:
1. Vous demander confirmation (tapez `y`)
2. Cloner le repository
3. Créer toute la structure nécessaire
4. Builder et pusher les 16 services (40-50 min)
5. Déployer sur ECS
6. Afficher toutes les IPs
...
Dans un autre terminal CloudShell, lancez le monitoring:
...
- ✅ 100% automatique
- ✅ **Utilise l'API GitHub** pour télécharger les Dockerfiles existants (évite le CDN 404)
- ✅ Crée les Dockerfiles manquants localement en fallback
- ✅ Crée toute la structure manquante (packages, infra/seeds)
- ✅ Gère les erreurs gracieusement
- ✅ Affiche les résultats finaux avec toutes les IPs
- ✅ Prêt à l'emploi immédiatement

---

**Copiez-collez simplement la commande `cat > ~/auto.sh` complète dans CloudShell et lancez `~/auto.sh` !** 🎉
